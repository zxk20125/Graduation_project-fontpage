<template>
    <v2container :wid="`root`" style="display:flex;align-items:flex-start;align-self:stretch;height:100%;box-sizing:border-box;animation-duration:1s;flex-direction:column;min-width:50px;min-height:50px;" class="V2Container">
        <div :wid="`1578537283708`" class="v2Container-1578537283708 V2Container" style="display:flex;align-items:flex-start;align-self:stretch;height:auto;box-sizing:border-box;position:relative;overflow:hidden;min-width:50px;min-height:50px;animation-duration:1s;flex-direction:row;flex-shrink:0;" ref="v2Container_1578537283708">
            <v2-form-input :wid="`1579152715570`" class="v2-form-input-1579152715570 V2Widget" :style="{'position':'relative','margin-top':'10px','margin-bottom':'10px','margin-left':'10px','margin-right':'10px','overflow':'hidden','animation-duration':'1s'}" :theme="{'size':'medium'}" :disabled="false" :label="`创建用户`" :placeholder="`请输入创建用户`" :title-mode="`col`" :label-witdh="`80px`" :input-type="`text`" :rows="2" :show-word-limit="false" :clearable="false" :is-required="false" :auto-complete="false" :value.sync="creator" ref="v2-form-input_1579152715570">
            </v2-form-input>
            <v2-form-input :wid="`1580789804791`" class="v2-form-input-1580789804791 V2Widget" :style="{'position':'relative','margin-top':'10px','margin-bottom':'10px','margin-left':'10px','margin-right':'10px','overflow':'hidden','animation-duration':'1s'}" :theme="{'size':'medium'}" :disabled="false" :label="`权限名称`" :placeholder="`请输入权限名称`" :title-mode="`col`" :label-witdh="`80px`" :input-type="`text`" :rows="2" :show-word-limit="false" :clearable="false" :is-required="false" :auto-complete="false" :value.sync="name" ref="v2-form-input_1580789804791">
            </v2-form-input>
        ...
        <div :wid="`1578539257540`" class="v2Container-1578539257540 V2Container" style="display:flex;align-items:flex-start;align-self:stretch;height:auto;box-sizing:border-box;position:relative;overflow:hidden;min-width:50px;min-height:50px;animation-duration:1s;flex-direction:row;" ref="v2Container_1578539257540">
            ...
            <v2-component-btn :wid="`1578539296885`" class="v2-component-btn-1578539296885 V2Widget" :style="{'position':'relative','margin-top':'10px','margin-bottom':'10px','margin-left':'10px','margin-right':'10px','overflow':'hidden','animation-duration':'1s'}" :disabled="isDisabled" :theme="{'btnType':'normal','size':'medium'}" :loading="false" :btn-name="`删除`" :shape="`plain`" @_op_component-btn_btn:click="v2_component_btn_1578838839000" ref="v2-component-btn_1578539296885">
            </v2-component-btn>
        </div>
        <v2-component-table :wid="`1578539272258`" class="v2-component-table-1578539272258 V2Widget" :style="{'position':'relative','margin-top':'10px','margin-bottom':'10px','margin-left':'10px','margin-right':'10px','flex-basis':'100%','overflow':'hidden','animation-duration':'1s','width':'calc(100% - 20px)','flex-shrink':0}" :tableData="authorityTableList" :pageSize="pageSize" :totalCount="authorityTotalCount" :theme="{'tableType':'border','size':''}" :table-data="authorityTableList" :page-size="pageSize" :total-count="authorityTotalCount" :columns="[{'active':true,'prop':'name','label':'权限名称','sortable':true,'openFilter':false,'type':'text','combi':[{'active':'true'}],'tagMap':[{'active':'true','type':'text'}],'iconMap':[{'active':'true'}],'filterMap':[{'active':'true'}]},{'active':true,'prop':'creator','label':'创建用户','sortable':true,'openFilter':false,'type':'text','combi':[{'active':'true'}],'tagMap':[{'active':'true','type':'text'}],'iconMap':[{'active':'true'}],'filterMap':[{'active':'true'}]},{'active':true,'prop':'createTime','label':'创建时间','sortable':true,'openFilter':false,'type':'text','combi':[{'active':'true'}],'tagMap':[{'active':'true','type':'text'}],'iconMap':[{'active':'true'}],'filterMap':[{'active':'true'}]},{'active':true,'prop':'editor','label':'最近修改用户','sortable':true,'openFilter':false,'type':'text','combi':[{'active':'true'}],'tagMap':[{'active':'true','type':'text'}],'iconMap':[{'active':'true'}],'filterMap':[{'active':'true'}]},{'active':true,'prop':'editTime','label':'最近修改时间','sortable':true,'openFilter':false,'type':'text','combi':[{'active':'true'}],'tagMap':[{'active':'true','type':'text'}],'iconMap':[{'active':'true'}],'filterMap':[{'active':'true'}]},{'active':true,'prop':'desp','label':'描述','sortable':false,'openFilter':false,'type':'text','combi':[{'active':'true'}],'tagMap':[{'active':'true','type':'text'}],'iconMap':[{'active':'true'}],'filterMap':[{'active':'true'}]}]" :selection="`multi`" :selection-location="`left`" :t-height="`100%`" :t-max-height="`100%`" :open-index="false" :open-page="true" :open-set-page-callback="true" :set-page-callback="`getPageAuthorityList`" :tool-bar="true" :tool-title="`操作列`" :tool-location="`right`" :tool-width="`100px`" :tool-btns="[{'active':true,'btnName':'编辑','type':'text','handler':'editAuthority','rowName':'','toolIcon':'','rowValue':''}]" :expand="false" :value.sync="selectedAuthorityList" @_op_table_table:selection-change="v2_component_table_1578928960000" ref="v2-component-table_1578539272258">
        </v2-component-table>
        ...
        'methods': {
            ...
            openAddAuthorityPage() {
                ...
                ctx.open({
                    title:'新增权限',
                    path:'/add/authority',
                    component:'authority/addAuthority',
                    type:'SUB',
                    ...
                    confirmCallback:(params) => { 
                        ...
                        ctx.$axios.post(`${axios.server}/authority/add`, params).then(res => {
                            //下一步的行为
                            if(res.success){
                                ctx.$notify({
                                    title: '新增权限成功',
            ...
            deleteAuthority() {
                ...
                authorityIds = ctx.selectedAuthorityList.map(function(item){
                    
                    return item.id;
                });
                
                ctx.$confirm('确定删除选中记录码？', '删除权限', {
                    distinguishCancelAndClose: true,
                    confirmButtonText: '确定',
                    cancelButtonText: '取消'
                }).then(() => {
                    ctx.$axios.post(`${axios.server}/authority/delete`, JSON.stringify(authorityIds),{
                        headers: {
            ...
            editAuthority(index, row) {
                ...
                ctx.open({
                    ...
                    hideCancelBtn:true, 
                    confirmCallback:(params) => { 
                        ctx.$axios.post(`${axios.server}/authority/update`, params).then(res => {
                        //下一步的行为
                            if(res.success){
                                ctx.$notify({
                                    title: '编辑权限成功',
                               ...
                               ctx.getPageAuthorityList(ctx.currentPage);
             ...
             *  @param 新参数 size
             */
            getPageAuthorityList(page, size) {
                //函数内直接用ctx代替this访问vue页面的数据。
                const ctx = this;
                ...
                ctx.$axios.post(`${axios.server}/authority/list`,params).then(res=>{
                    if(res.success&&res.status){
                      ...
                      ctx.authorityTotalCount = res.obj&&res.obj.totalElements;
                    }
                }).catch(error=>{
            ...
            /**
             *  表格复选框状态改变时获取表格选中数据
             *  @param 参数selection selections
             *  @param 组件的vue实例 vueIns
             */
            v2_component_table_1578928960000(selections, vueIns) {
                /**
                * 事件绑定在vue实例上，第一个参数是vue实例。
                * 更多其他参数可以参考element-ui官网:https://element.eleme.cn/#/zh-CN
                **/
                ...
                console.log(123)
                debugger
                ctx.selectedAuthorityList = selections||[];
            ...
            v2_pagination_1578929249000($event, vueIns=this.$refs['v2-pagination_1578838455979'], currentPage) {
                ...
                **/
                ...
                ctx.getPageAuthorityList(currentPage);
            },
            /**
